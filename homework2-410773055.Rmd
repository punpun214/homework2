...
```{r ans22}
commit8Date <- ymd_hms("2019-04-24T00:58:55Z")
...
```{r ans26}
hour <- hour(allCommitDatesTW)
minute <- minute(allCommitDatesTW)
hour1 <- as.numeric(hour) 
minute1<- as.numeric(minute)
x <- (hour1 == "5") | (hour1 == "6") & (minute1 == "1":"30")
x
y <- (hour1 == "4") & (minute1 == "33")
y
logicWithinTimeFrame <- x | y
...
variableNames <- as.character(transcriptData)
variableNames
```
...
```{r ans32}
b1 <- transcriptData[["學系"]]
logicFromEcon <- ( b1 == "經濟學系")
logicFromEcon
```
